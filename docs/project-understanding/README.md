...
Progress belajar disimpan pada pivot `enrollments.progress`. Saat user menyelesaikan materi melalui `completeLesson`, progress bertambah 10% sampai maksimal 100%. Jika progress mencapai 100%, status enrollment berubah dari `active` menjadi `finished`.
...
- `DatabaseSeeder` saat ini membuat akun admin dan user, tetapi tidak memanggil `CourseSeeder`; jika ingin data kursus otomatis, tambahkan `$this->call(CourseSeeder::class)`.
- Beberapa komentar kode masih campuran Indonesia dan placeholder, terutama pada API course/notes.
- Approval pembayaran memakai `attach`; jika pembayaran disetujui dua kali untuk user-course yang sama, perlu proteksi duplicate enrollment.
- `Payment::$fillable` belum mencantumkan `rejection_reason`, meskipun field dipakai saat reject.
