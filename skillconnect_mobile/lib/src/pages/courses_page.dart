...
class CoursesPage extends StatefulWidget {
  const CoursesPage({super.key, this.token, this.onUnauthorized});
...
class _CoursesPageState extends State<CoursesPage> {
  ...
  late Future<List<Course>> _coursesFuture = _loadCourses();

  Future<List<Course>> _loadCourses() async {
    try {
      final publicCourses = await _repository.fetchCourses();
      final token = widget.token;
      if (token == null) return publicCourses;

      final ownedCourses = await _repository.fetchMyCourses(token);
      final ownedById = {for (final course in ownedCourses) course.id: course};
      return [
        for (final course in publicCourses) ownedById[course.id] ?? course,
      ];
